...
import { 
  ...
  Download, Calendar, Users, Briefcase, CreditCard 
} from 'lucide-react';
...
export default function RevenueAnalyticsPage() {
  ...
  const handleExport = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`/api/admin/revenue/export?days=${dateRange}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Monitor platform financial performance</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className={`flex items-center mt-2 text-sm ${
              ...
              <span className="ml-1">{Math.abs(stats.monthlyGrowth)}% vs last period</span>
            </div>
          </div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        viewMode === 'revenue'
                          ? (data.revenue / Math.max(...stats.monthlyData.map(d => d.revenue))) * 100
                          : (data.bookings / Math.max(...stats.monthlyData.map(d => d.bookings))) * 100
                      }%`
                    }}
                  />
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Top Providers */}
          <div className="bg-white rounded-xl shadow-sm border p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Top Performing Providers</h2>
            <div className="space-y-4">
              {stats.topProviders.map((provider, index) => (
                <div key={provider.id} className="flex items-center justify-between p-4 hover:bg-gray-50 rounded-lg transition">
                ...
                <div key={category.category}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-600">{category.bookings} bookings</p>
                    </div>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${
                          (category.revenue / Math.max(...stats.revenueByCategory.map(c => c.revenue))) * 100
                        }%`
                      }}
                    />
                  </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-sm p-6 text-white">
            ...
            <p className="text-3xl font-bold">KSH {stats.weeklyRevenue.toLocaleString()}</p>
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-sm p-6 text-white">
            ...
            <p className="text-3xl font-bold">KSH {stats.todayRevenue.toLocaleString()}</p>
